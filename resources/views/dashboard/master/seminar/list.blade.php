        ...
        <div class="row page-titles mx-0">
            <div class="col p-md-0">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"><a href="{{route('seminar.index')}}">{{ $breadcrumb }}</a></li>
                </ol>
            </div>
        </div>
        <!-- row -->
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="pb-3 d-flex justify-content-between">
                                <a href="{{ route('seminar.create') }}" class="btn mb-4 mt-2 btn-info">+ Tambah Data</a>
                                <div class="search">
                                    <form action="{{ route('seminar.index') }}" method="GET" class="d-flex">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered zero-configuration">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($data_seminar as $key => $item)
                                            <tr>
                                                <td>{{ $data_seminar->firstItem() + $key }}</td>
                                                <td>{!! $item->judul !!}</td>
                                                <td>{!!Substr($item['deskripsi'], 0,500)!!}...</td>
                                                <td><img src="{{ asset('storage/seminar/' . $item->foto) }}"
                                                ...
                                                <td>{!! $item->status !!}</td>
                                                    <td>
                                                    <a href="{{ route('seminar.edit', $item->id) }}"
                                                ...
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-footer">
                                                            <form action="{{ route('seminar.destroy', $item->id) }}"
                                ...
                                <div>
                                    Showing 
                                    {{$data_seminar->firstItem()}}
                                    to
                                    {{$data_seminar->lastItem()}}
                                    of
                                    {{$data_seminar->total()}}
                                </div>
