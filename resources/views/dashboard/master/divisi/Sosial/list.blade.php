        ...
        <div class="row page-titles mx-0">
            <div class="col p-md-0">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"><a href="{{route('sosial.index')}}">{{ $breadcrumb }}</a></li>
                </ol>
            </div>
        </div>
        <!-- row -->
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="pb-3 d-flex justify-content-between">
                                <a href="{{ route('sosial.create') }}" class="btn mb-4 mt-2 btn-info">+ Tambah Data</a>
                                <div class="search">
                                    <form action="{{ route('sosial.index') }}" method="GET" class="d-flex">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered zero-configuration">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($data_sosial as $key => $item)
                                            <tr>
                                                <td>{{ $data_sosial->firstItem() + $key }}</td>
                                                <td>{!! $item->nama !!}</td>
                                                <td>{!! $item->angkatan !!}</td>
                                                <td>{!! $item->jabatan !!}</td>
                                                <td><img src="{{ asset('storage/divisi-sosial/' . $item->foto) }}"
                                ...
                                <div>
                                    Showing 
                                    {{$data_sosial->firstItem()}}
                                    to
                                    {{$data_sosial->lastItem()}}
                                    of
                                    {{$data_sosial->total()}}
                                </div>
