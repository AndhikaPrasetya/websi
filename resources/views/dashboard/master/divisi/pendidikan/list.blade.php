        ...
        <div class="row page-titles mx-0">
            <div class="col p-md-0">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"><a href="{{route('pendidikan.index')}}">{{ $breadcrumb }}</a></li>
                </ol>
            </div>
        </div>
        <!-- row -->
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="pb-3 d-flex justify-content-between">
                                <a href="{{ route('pendidikan.create') }}" class="btn mb-4 mt-2 btn-info">+ Tambah Data</a>
                                <div class="search">
                                    <form action="{{ route('pendidikan.index') }}" method="GET" class="d-flex">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered zero-configuration">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($data_pendidikan as $key => $item)
                                            <tr>
                                                <td>{{ $data_pendidikan->firstItem() + $key }}</td>
                                                <td>{!! $item->nama !!}</td>
                                                <td>{!! $item->angkatan !!}</td>
                                                <td>{!! $item->jabatan !!}</td>
                                                <td><img src="{{ asset('storage/divisi-pendidikan/' . $item->foto) }}"
                                ...
                                <div>
                                    Showing 
                                    {{$data_pendidikan->firstItem()}}
                                    to
                                    {{$data_pendidikan->lastItem()}}
                                    of
                                    {{$data_pendidikan->total()}}
                                </div>

                                   <div class="pull-right">
                                    {{ $data_pendidikan->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
