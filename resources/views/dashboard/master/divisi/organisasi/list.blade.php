        ...
        <div class="row page-titles mx-0">
            <div class="col p-md-0">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"><a href="{{route('organisasi.index')}}">{{ $breadcrumb }}</a></li>
                </ol>
            </div>
        </div>
        <!-- row -->
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="pb-3 d-flex justify-content-between">
                                <a href="{{ route('organisasi.create') }}" class="btn mb-4 mt-2 btn-info">+ Tambah Data</a>
                                <div class="search">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered zero-configuration">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($data_organisasi as $key => $item)
                                            <tr>
                                                <td>{{ $data_organisasi->firstItem() + $key }}</td>
                                                <td>{!! $item->nama !!}</td>
                                                <td>{!! $item->angkatan !!}</td>
                                                <td>{!! $item->jabatan !!}</td>
                                                <td><img src="{{ asset('storage/divisi-organisasi/' . $item->foto) }}"
                                ...
                                <div>
                                    Showing 
                                    {{$data_organisasi->firstItem()}}
                                    to
                                    {{$data_organisasi->lastItem()}}
                                    of
                                    {{$data_organisasi->total()}}
                                </div>

                                   <div class="pull-right">
                                    {{ $data_organisasi->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
